...
public class Server extends Thread {
     ...
     */
     ...
     {   int accIndex = 0;             	/* Index position of account to update */
         ...
         { 
             ...
             // User Code Begins
             // Yield if input buffer is empty
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
        		...
        		// System.out.println("\n DEBUG : Server.processTransactions() - transferring out account " + trans.getAccountNumber());
                 ...
                 // disconnect from network after processing a transaction on every account
                 if (getNumberOfTransactions() == getNumberOfAccounts()){
    ...
    {   Transactions trans = new Transactions();
        ...
        long serverRT = System.currentTimeMillis();

    	// System.out.println("\n DEBUG : Server.run() - starting server thread " + objNetwork.getServerConnectionStatus());
    	/* Implement the code for the run method */
        processTransactions(trans);
        System.out.println("\n Terminating server thread - " + " Running time " + (System.currentTimeMillis() - serverRT) + " milliseconds");

        // Disconnect from network
        objNetwork.setServerConnectionStatus("disconnected");

        // Placed so that the output looks better
        // I know that this should be in the Network run() but I can't get it to work properly
        if((objNetwork.getClientConnectionStatus().equals("disconnected")) && objNetwork.getServerConnectionStatus().equals("disconnected")){
